...
const MovieDetail = () => {
  ...
  const filteredShowtimes = showtimes.filter(showtime => 
    ...
    (!selectedCluster || showtime.clusterId === selectedCluster)
  );

  const availableDates = [...new Set(showtimes.map(s => s.date))].sort();
  const availableClusters = [...new Set(showtimes.map(s => s.clusterId))];
                  ...
                  <div className="max-w-4xl">
                    ...
                    <div className="flex flex-wrap items-center gap-6 mb-6">
                      <div className="flex items-center">
                        ...
                        <span className="text-white font-bold text-xl">{movie.imdbRating}/10</span>
                      </div>
                    ...
                    <div className="flex flex-wrap gap-3 mb-8">
                      {movie.genre.map((g, index) => (
                        <span key={index} className="px-4 py-2 bg-purple-500/30 text-purple-200 text-sm rounded-full border border-purple-400/50">
                          {g}
                        </span>
                      ))}
                    </div>
              ...
              <div className="lg:col-span-2 space-y-8">
                ...
                <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-8 border border-white/20">
                  ...
                  <div className="space-y-4">
                    <div>
                      <h4 className="text-lg font-semibold text-white mb-3">Đạo diễn</h4>
                      <p className="text-gray-300 text-lg">{movie.director}</p>
                    </div>
                    <div>
                      <h4 className="text-lg font-semibold text-white mb-3">Diễn viên chính</h4>
                      <div className="flex flex-wrap gap-3">
                        {movie.cast.map((actor, index) => (
                          <span key={index} className="px-4 py-2 bg-purple-500/20 text-purple-300 text-sm rounded-full border border-purple-400/30 hover:bg-purple-500/30 transition-colors">
                            {actor}
                          </span>
                        ))}
                      </div>
              ...
              <div className="space-y-6">
                <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-white">{new Date(movie.releaseDate).toLocaleDateString('vi-VN')}</span>
                    </div>
                    <div className="flex justify-between">
                ...
                <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {movie.genre.map((g, index) => (
                      <span key={index} className="px-3 py-1 bg-gradient-to-r from-purple-500/20 to-pink-500/20 text-purple-300 text-sm rounded-full border border-purple-400/30">
                        {g}
                      </span>
                    ))}
                  </div>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                ...
                <div>
                  <label className="block text-white font-medium mb-2">Chọn rạp</label>
                  <select
                    value={selectedCluster}
                    onChange={(e) => setSelectedCluster(e.target.value)}
                    className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                  >
                    <option value="" className="bg-slate-800">Tất cả rạp</option>
                    {availableClusters.map(clusterId => (
                      <option key={clusterId} value={clusterId} className="bg-slate-800">
                        Rạp {clusterId}
                      </option>
                    ))}
                  </select>
              ...
              {filteredShowtimes.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {filteredShowtimes.map((showtime) => (
                    <div key={showtime.showtimeId} className="bg-white/5 border border-white/10 rounded-xl p-4 hover:bg-white/10 transition-all duration-300">
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          <h3 className="text-white font-semibold">Rạp {showtime.clusterId}</h3>
                          <p className="text-gray-300 text-sm">Phòng {showtime.hallId}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-white font-bold text-lg">{formatPrice(showtime.price)}</p>
                          <p className="text-gray-300 text-sm">{showtime.availableSeats}/{showtime.totalSeats} ghế</p>
                        </div>
                      </div>
                      <div className="flex items-center justify-between">
                        <div className="text-center">
                          <p className="text-white font-bold text-xl">{showtime.startTime}</p>
                          <p className="text-gray-300 text-sm">Kết thúc: {showtime.endTime}</p>
                        </div>
                        <button
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="text-xl font-semibold text-white mb-2">Không có suất chiếu</h3>
                  <p className="text-gray-300">Không có suất chiếu nào cho ngày và rạp đã chọn</p>
                </div>
              )}
            </div>
          </div>
